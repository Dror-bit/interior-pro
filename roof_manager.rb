...
module InteriorPro
  module RoofManager
    ...
    def self.build_roof!(style: nil, pitch: nil, overhang: nil,
      ...
      # the gable triangle itself: a vertical face closing every gable end
      # (the framed path already built its triangles per wing)
      if zmap && framed.nil?
        gables.each { |i| build_gable_wall_face!(grp, poly, i, zmap) }
      end
      if s[:fascia]
    ...
    def self.framed_plan(poly, wall_ids, marked, style)
      ...
      cands = []
      [:y, :x].each do |axis|
        rects = slab_rects(poly, axis)
        next unless rects
        area = rects.sum { |x0, y0, x1, y1| (x1 - x0) * (y1 - y0) }
        next if (area - polygon_area(poly).abs).abs > 2.0
        pl = assemble_framed_plan(rects, poly, wall_ids, marked, style)
        cands << pl if pl
      end
      return nil if cands.empty?
      cands.max_by { |pl| [pl[:score], -pl[:nrects]] }
    end

    def self.assemble_framed_plan(rects, poly, wall_ids, marked, style)
      return nil if rects.empty? || rects.length > 6
      ...
      rects.each_with_index do |r, i|
        next if i == main_i
        m = mouth_side(r, main)
        return nil unless m # wing-on-wing: not supported, use fallback
        gab = side_gabled?(r, OPP_SIDE[m], poly, wall_ids, marked)
        wings << { rect: r, mouth: m, gabled: gab }
      end
      g = {}
      [:n, :s, :e, :w].each { |sd| g[sd] = side_gabled?(main, sd, poly, wall_ids, marked) }
      if style == 'gable' && marked.empty? && !g.values.any?
        # auto Gable style with no marks: the two short ends of the main
    ...
    # vertical gable triangle: unpainted, the catch-all pass paints trim
    def self.framed_tri!(st, pts3)
      ...
      st[:grp].entities.add_face(pts3.map { |(x, y, z)| Geom::Point3d.new(x, y, z) })
    end
