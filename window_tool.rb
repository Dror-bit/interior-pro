...
module InteriorPro
  class WindowTool
    ...
    def onLButtonDown(flags, x, y, view)
      ...
      unless wall
        msg = if @off_wall_pick
                'Not on a wall face. Click directly on the wall you want the window in.'
              else
                'No wall under cursor. Hover over a wall to place a window.'
              end
    ...
    # A click, moved out of WORLD space and into the wall group's OWN space.
    #
    # start_x/start_y/end_x/end_y are LOCAL to the wall group; a pick is in
    # world space. While a wall sits at the model origin the two are the same
    # numbers and nobody notices. A wall on level 2 - or any wall that was
    # copied, moved or welded - carries a shift in its group transformation,
    # and then they are not. Measured on the user's own house
    # (win_level_report.txt, 2026-08-18): 10 of his 12 level-2 walls sit at
    # y = -161.687, and 18 of his 22 level-1 walls carry a shift too. Only the
    # 4 identity walls were ever computed correctly - which is exactly why the
    # ground floor "worked better" instead of working.
    #
    # A wall lifted in Z only comes back with the same x/y it went in with, so
    # this is a no-op for every case that already behaved. Rotation and scale
    # are handled for free because the inverse matrix does the work.
    #
    # BOTH the filter (wall_click_offsets) and the cut (cut_window_opening)
    # must go through here. If only one of them converts, the filter stops
    # predicting the cut and is worth nothing. rt74 fails if either drops it.
    def wall_local_point(wall_group, world_pt)
      return nil unless wall_group && world_pt
      begin
        world_pt.transform(wall_group.transformation.inverse)
      rescue StandardError => e
        puts "[WindowTool] wall_local_point: #{e.message}"
        world_pt
      end
    end

    # How far a clicked WORLD point sits off this wall's centreline plane, and
    # how far along the wall it lands.
    #
    # Computed EXACTLY the way cut_window_opening computes it - same
    # attributes, same anchor offset, same sign convention - so this PREDICTS
    # what the cut will do instead of guessing at it. If the two ever drift
    # apart, the filter below is worthless. Keep them together.
    def wall_click_offsets(wall_group, world_pt)
      return nil unless wall_group && wall_group.valid? && world_pt
      ...
      th = wall_group.get_attribute('InteriorPro', 'thickness').to_f
      return nil unless sx && sy && ex && ey && th > 0

      drawn_start = Geom::Point3d.new(sx, sy, 0)
      drawn_end   = Geom::Point3d.new(ex, ey, 0)
      vec = drawn_end - drawn_start
      len = vec.length
      ...
      _v, h_anchor = parse_anchor(wall_group.get_attribute('InteriorPro', 'anchor') ||
                                  'bottom-center')
      center_offset = case h_anchor
                      when 'left'  then th / 2.0
                      when 'right' then -th / 2.0
                      else 0.0
                      end
      cline_start = Geom::Point3d.new(drawn_start.x + n.x * center_offset,
                                      drawn_start.y + n.y * center_offset, 0)
      local_pt = wall_local_point(wall_group, world_pt)
      return nil unless local_pt
      to_click = Geom::Point3d.new(local_pt.x, local_pt.y, 0) - cline_start
      { t: to_click.dot(unit), n_offset: to_click.dot(n),
        length: len, thickness: th }
    end

    # 3" of slack past the wall face: room for board and batten, siding and a
    # casing, and nowhere near enough to reach the next wall.
    def off_wall_tol
      3.0
    end

    # Is the click really ON this wall? A curved wall bows away from the
    # straight line between its ends, so it is allowed its whole sag on top.
    def click_on_wall?(wall_group, world_pt)
      o = wall_click_offsets(wall_group, world_pt)
      return false if o.nil?
      allow = o[:thickness] / 2.0 + off_wall_tol
      begin
        if defined?(InteriorPro::WallTool) &&
           InteriorPro::WallTool.curved_wall?(wall_group)
          allow += InteriorPro::WallTool.wall_sag(wall_group).to_f.abs
        end
      rescue StandardError
        nil
      end
      o[:n_offset].abs <= allow
    end
    ...
    def find_wall_under_cursor(view, x, y)
      ...
      ph.count.times do |i|
        ...
        next unless point
        # SketchUp hands back EVERYTHING along the pick ray, near and far.
        # This loop used to take the first path that merely CONTAINED a
        # wall, so when the nearest thing was not a wall - a pillar, an
        # existing window, the roof overhang - it walked deeper and grabbed
        # a wall on the FAR SIDE of the house, then cut the hole there.
        # Measured on the user's model 2026-08-18: n_offset 87.88", i.e.
        # the wall it chose was seven feet behind the point he clicked.
        unless click_on_wall?(wall, point)
          @off_wall_pick = true
          next
        end
        return [wall, point, face]
      end
      [nil, nil, nil]
    end
    ...
    def cut_window_opening(wall_group, picked_point, picked_face = nil)
      ...
      # The pick is in WORLD space, everything below is LOCAL to the wall
      # group - see wall_local_point. Level 2 walls (and copied/moved walls)
      # carry a plan shift, so these are not the same numbers. The filter
      # in wall_click_offsets converts identically; the two MUST stay together.
      local_click = wall_local_point(wall_group, picked_point) || picked_point
      click_xy = Geom::Point3d.new(local_click.x, local_click.y, 0)
      ...
      # Belt and braces. find_wall_under_cursor already filters, but a hole
      # in the wrong wall is expensive to undo and cheap to refuse, and this
      # method has other callers.
      allow_off = thickness / 2.0 + off_wall_tol + (curved ? wall_sag.abs : 0.0)
      if n_offset.abs > allow_off
        UI.messagebox(
          "That point is not on this wall.\n\n" \
          "It is #{n_offset.abs.round(1)}\" off the wall face, and at most " \
          "#{allow_off.round(1)}\" is allowed.\n\n" \
          'Click directly on the wall you want the window in.'
        )
        return
      end
      ...
      begin
        ...
        # cx/cy came out of cline_start, so they are in the WALL's space, while
        # this group is a sibling of the wall. Push them through the wall's own
        # transformation instead of only adding wall_z by hand - otherwise the
        # marker lands where the wall was DRAWN rather than where it stands.
        # For an identity or purely lifted wall this is the same point as
        # before, digit for digit (2026-08-18C §8).
        window_group.transformation = Geom::Transformation.new(
          Geom::Point3d.new(cx, cy, (win_bot_z + win_top_z) / 2.0)
                         .transform(wall_group.transformation)
        )
