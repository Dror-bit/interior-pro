...
module InteriorPro
  module LevelManager
    ...
    # Above level 1 the ceilings of the rooms below MUST exist (user rule
    # 2026-08-03) - any missing one is built automatically.
    ...
    # Every room of the level below gets a ceiling if it has none yet.
    # Rooms that already have one are left exactly as they are.
    def self.ensure_ceilings_below!
      return 0 unless defined?(InteriorPro::CeilingManager) && defined?(InteriorPro::RoomManager)
      n = active_level
      rooms = InteriorPro::RoomManager.rooms_in_model.select do |r|
        (r.get_attribute('InteriorPro', 'level') || 1).to_i < n
      end
      return 0 if rooms.empty?
      have = InteriorPro::CeilingManager.ceilings_in_model.map do |c|
        c.get_attribute('InteriorPro', 'room_id')
      end
      missing = rooms.reject { |r| have.include?(r.get_attribute('InteriorPro', 'id')) }
      return 0 if missing.empty?
      built = missing.count { |r| InteriorPro::CeilingManager.build_ceiling_for_room!(r) }
      puts "[Levels] auto-built #{built} ceiling(s) under the new level" if built > 0
      built
    rescue StandardError => e
      puts "[Levels] ensure_ceilings_below!: #{e.message}"
      0
    end
    ...
    # Auto hook (2026-08-04, user request): as soon as walls exist above
    # level 1, the structure between the levels is built/refreshed by
    # itself - level-1 exterior walls rise to the level-2 floor and the
    # subfloor deck closes the gap. Called AFTER the wall operation
    # commits (build_level2_structure! opens its own operation).
    def self.ensure_structure_below!
      return 0 if active_level <= 1
      return 0 if exterior_walls.empty?
      build_level2_structure!
    end
