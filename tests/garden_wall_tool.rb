...
module InteriorPro
  module Landscape
    class GardenWallTool
      ...
      # THE ELEVATION LINE (step 4, 2026-08-17). His words: "אני גם רוצה
      # שבתור התחלה יהיה קו מקביל לחלק האחורי שלה שזז איתה כשהיא עיגול או
      # יורדת למטה או עולה למעלה, כדי שאני אוכל לקחת את הקו הזה ולייצא איתו
      # אלווישן עד שנכין את המוצר לאלווישן."
      #
      # So: one edge running the length of the wall, along the top of the BACK
      # face, UNDER the cap (he was specific about that when asked). It is a
      # working line, not geometry - something to grab and pull an elevation
      # off until the real elevation tool exists.
      #
      # BACK is the LEFT of start -> end, because the bullnose and the
      # overhang are both on the right: the right is the face that shows, so
      # the back is the other one. Draw the wall the other way and all three
      # swap together.
      #
      # It gets its OWN tag, not the wall's, so he can switch every one of
      # them off in one click when he is not drawing elevations.
      BACKLINE_NAME = 'LandscapePro_GardenWallBackLine' unless const_defined?(:BACKLINE_NAME, false)
      BACKLINE_TAG  = 'LP/Wall Elevation Lines'         unless const_defined?(:BACKLINE_TAG, false)
      BACKLINE_TYPE = 'garden_wall_back_line'           unless const_defined?(:BACKLINE_TYPE, false)
      ...
      def initialize(thickness = DEFAULT_THICKNESS, height = DEFAULT_HEIGHT,
        ...
        @back_line    = back_line ? true : false
        @ground_start = 0.0
        @ground_end   = 0.0
        reset
      end
      ...
      # THE TWO ENDS OF THE ELEVATION LINE, in world space. Pure, so where it
      # runs can be checked without SketchUp.
      #
      # Along the top of the BACK face - the left of start -> end, opposite
      # the nose and the overhang - and at the top of the WALL, which is under
      # the cap, not on top of it.
      #
      # Everything it has to follow later is already decided here: when the
      # wall bows this becomes a run of points along the same offset, and when
      # it steps up or down the z comes from the ground at each end instead of
      # one flat height. Both go in this method and nowhere else.
      def back_line_points(a, b)
        u, n = self.class.axes(a, b)
        return nil unless u
        up = Geom::Vector3d.new(0, 0, 1)
        half = @thickness.to_f / 2.0
        [a.offset(n, -half).offset(up, @height),
         b.offset(n, -half).offset(up, @height)]
      end
      ...
      def self.prompt!
        res = UI.inputbox(
          ['Thickness', 'Height', 'Finish',
           ...
           'Bullnose', 'Cap overhang', 'Elevation line along the back'],
      ...
      def build!(a, b, model = Sketchup.active_model)
        ...
        build_back_line!(group, a, b) if @back_line

        stamp!(group, a, b)
        group
      end

      # One edge, its own group, its own tag. A group so he can click it once
      # and get the whole line; its own tag so every elevation line in the
      # model switches off together.
      def build_back_line!(parent, a, b)
        pts = back_line_points(a, b)
        return nil unless pts
        return nil if pts[0].distance(pts[1]) <= 0.0

        line = parent.entities.add_group
        line.name = BACKLINE_NAME
        line.entities.add_line(pts[0], pts[1])
        line.set_attribute(DICT, 'type', BACKLINE_TYPE)
        begin
          InteriorPro.assign_tag(line, BACKLINE_TAG)
        rescue StandardError
          nil
        end
        line
      rescue StandardError => e
        puts "[GardenWall] back line: #{e.class}: #{e.message}"
        nil
      end
      ...
      def stamp!(group, a, b)
        ...
        group.set_attribute(DICT, 'back_line', @back_line ? true : false)
