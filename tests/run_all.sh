...
for f in fence_math.rb fence_tool.rb fence_library.rb fence_library_dialog.rb fence_ref_tool.rb; do
  [ -f "$SRC/landscape/$f" ] && cp "$SRC/landscape/$f" . || echo "  (missing landscape/$f - some suites will be skipped)"
done

echo "== ruby syntax =="
fail=0
for f in plan_doc.rb plan_tables.rb plan_canvas.rb plan_pdf.rb plan_editor.rb plan_generator.rb state_backup.rb auto_backup.rb door_library.rb door_tool.rb arc_math.rb door_manager.rb wall_tool.rb fence_math.rb fence_tool.rb fence_library.rb fence_library_dialog.rb fence_ref_tool.rb; do
  [ -f "$f" ] || continue
