...
for f in plan_editor.rb plan_generator.rb door_library.rb door_tool.rb room_manager.rb ceiling_manager.rb level_manager.rb molding_tool.rb wall_tool.rb foundation_manager.rb ui_dialogs.rb wall_stretch_tool.rb floor_manager.rb roof_manager.rb roof_dialog.rb; do
  [ -f "$SRC/$f" ] && cp "$SRC/$f" . || echo "  (missing $f - some suites will be skipped)"
done

echo "== ruby syntax =="
fail=0
for f in plan_editor.rb plan_generator.rb door_library.rb door_tool.rb; do
  [ -f "$f" ] || continue
...
for r in rt rt2 rt3 rt4 rt5 rt6 rt7 rt8 rt9 rt10 rt11 rt12 rt13 rt14 rt15 rt16 rt17 rt18; do
  [ -f "$r.rb" ] || continue
